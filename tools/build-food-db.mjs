...
import { applyFastfoodOverlay, applyChainReclassification, applyDisplayNames, loadJsonIfExists } from './db/overlays.mjs';
...
// ---------- Overlay stage ----------
// Branded fast-food rows and consumer display names are curated/generated by
// hand (tools/db/curate-fastfood.mjs, tools/db/generate-display-names.mjs) --
// build-food-db.mjs has no way to derive either from the raw USDA CSVs. They
// used to only be applied to the shipped DB after the fact (merge-fastfood.mjs
// --apply / generate-display-names.mjs --apply), so a rebuild from source
// would silently lose both. Folding them in here keeps a rebuild reproducible.
// Both inputs are optional (existsSync-guarded in overlays.mjs) so a build
// still succeeds, minus the overlay, before either has been generated.
console.log('Applying overlays...');
const fastfoodItems = loadJsonIfExists(
  join(ROOT, 'db-data', 'fastfood-curated.json'),
  'fastfood-curated.json (branded overlay)'
);
if (fastfoodItems) {
  const r = applyFastfoodOverlay(db, fastfoodItems);
  console.log(`  branded rows: ${r.before} -> ${r.inserted} (${r.skipped} skipped as dup name_norm)`);
}
const chain = applyChainReclassification(db);
console.log(`  reclassified ${chain.changed} FNDDS chain rows as branded`);

const displayNames = loadJsonIfExists(
  join(ROOT, 'db-data', 'display-names.json'),
  'display-names.json (display_name overlay)'
);
if (displayNames) {
  const r = applyDisplayNames(db, displayNames);
  console.log(`  applied ${r.applied} display names`);
}
